...
from backend.services.agent_service import get_agent_graph
...
class RAGService:
    ...
    async def stream_query(
        ...
        # Since LangGraph streaming can be complex, we'll run the graph and yield words (simulated streaming)
        # OR we could just stream the llm generation inside the research node.
        # To keep it simple and reliable for now, run the graph and stream the final answer
        agent = get_agent_graph()
        result = await agent.ainvoke({
            "query": query,
            "document_ids": document_ids,
