  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex gap-3 group">
      ...
      <div className="flex-1 min-w-0">
        ...
        {sources.length > 0 && (
          <div className="mt-3">
            <button 
              ...
              className="text-[10px] uppercase tracking-widest text-indigo-400 font-bold flex items-center gap-1 hover:text-indigo-300 transition-colors"
            >
              <FileText size={12} />
              {showEvidence ? 'Hide Evidence' : `View ${sources.length} Sources`}
              ...
              {showEvidence && (
                ...
                >
                  <div className="grid grid-cols-1 gap-2 mt-2">
                    {sources.map((s, i) => (
                      <div key={i} className="glass p-3 rounded-xl border border-white/5 bg-white/[0.02]">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2 text-[10px] text-slate-400 font-medium">
                            <FileText size={10} className="text-indigo-400" />
                            <span className="truncate max-w-[200px] font-display">{s.filename}</span>
                            <span className="bg-indigo-500/10 text-indigo-300 px-1.5 py-0.5 rounded text-[9px]">Chunk {s.chunk_index + 1}</span>
                          </div>
                          {s.similarity != null && (
                            <div className="text-[9px] text-slate-500">Relevance: {Math.round(s.similarity * 100)}%</div>
                          )}
                        </div>
                        {s.preview && (
                          <p className="text-[11px] text-slate-400 line-clamp-3 leading-relaxed italic border-l-2 border-indigo-500/30 pl-2 py-0.5">
                            "{s.preview}"
                          </p>
                        )}
                      </div>
        ...
        {/* Confidence + actions */}
        <div className="mt-2 flex items-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity flex-wrap">
          {msg.confidence != null && (
            <div className="flex flex-col gap-1 w-32">
               ...
               <div className="w-full bg-white/10 h-1.5 rounded-full overflow-hidden">
                 <div 
                   className={`h-full ${msg.confidence > 75 ? 'bg-emerald-500' : msg.confidence > 50 ? 'bg-yellow-500' : 'bg-red-500'}`} 
                   style={{width: `${Math.round(msg.confidence)}%`}}
                 />
               </div>
            </div>
          )}
